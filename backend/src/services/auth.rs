...
pub async fn check_login_lockout(pool: &sqlx::PgPool, key: &str, window_minutes: i64, limit: i32) -> Result<bool> {
    ...
    let (count,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM login_failures WHERE key_hash = $1 AND attempted_at > $2")
        .bind(key).bind(window_start).fetch_one(pool).await.map_err(AppError::Database)?;
...
pub fn create_access_token(user_id: &str, username: &str, secret: &str, expire_minutes: i64) -> Result<(String, String)> {
    let (encoding_key, _) = build_jwt_keys(secret)?;
    let now = Utc::now();
    let token_family = Uuid::new_v4().to_string();
    let jti = Uuid::new_v4().to_string();
    let claims = TokenClaims {
        sub: user_id.into(), username: username.into(),
        exp: (now + Duration::minutes(expire_minutes)).timestamp(),
        nbf: now.timestamp() - 5, iat: now.timestamp(),
        jti: jti.clone(), token_family: token_family.clone(),
...
pub fn create_refresh_token(user_id: &str, username: &str, secret: &str, token_family: &str) -> Result<(String, String)> {
    let (encoding_key, _) = build_jwt_keys(secret)?;
    let now = Utc::now();
    let jti = Uuid::new_v4().to_string();
    let claims = TokenClaims {
        ...
        nbf: now.timestamp() - 5, iat: now.timestamp(),
        jti: jti.clone(), token_family: token_family.into(),
...
pub async fn verify_and_consume_exchange_code(pool: &sqlx::PgPool, code: &str) -> Result<Option<(String, String, String)>> {
    ...
    if let Some(record) = record {
        ...
        Ok(Some((record.user_id, record.username, record.access_token)))
    } else { Ok(None) }
}

pub async fn create_exchange_code(pool: &sqlx::PgPool, code: &str, user_id: &str, username: &str, access_token: &str, ttl_seconds: i64) -> Result<()> {
    let id = Uuid::new_v4().to_string();
